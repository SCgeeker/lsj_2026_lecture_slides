...
dice_df <- data.frame(
  face     = 1:6,
  observed = as.integer(dice_table),
  expected = rep(300/6, 6)
)
knitr::kable(dice_df,
             col.names = c("點數", "觀測次數", "預期次數"),
             align = "c",
             digits = 1,
...
dice_plot_df <- data.frame(
  face  = factor(rep(1:6, 2)),
  count = c(as.integer(dice_table), rep(300/6, 6)),
  type  = rep(c("觀測值", "預期值"), each = 6)
)

ggplot(dice_plot_df, aes(x = face, y = count, fill = type)) +
